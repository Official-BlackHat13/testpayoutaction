...
class PayoutService:
    ...
    def excutePAYTM(self, mode_rec):
        ...
        try:
            ...
            if(valid):
                ...
                log = Log_Model_Service(json=str({"merchant_id": self.merchant_id}), log_type="Checking for duplicate order id", client_ip_address=self.client_ip_address,
                                        server_ip_address=const.server_ip, remarks="checking for duplicate order id for merchant id :: " + self.merchant_id+" orderid :: "+str(payoutrequestmodel.orderId))
                log.save()
            ...
            #  ledgerModelService.customer_ref_no=order_id
                ...
                ledgerModelService.update_status(
                    id, 'Requested', client_ip_address=self.client_ip_address, created_by="Merchant_Id :: "+str(self.merchant_id))
                ledger_id = id
                tax_charges = Charge_Breaking_model_services(
                    charge_amount=taxes[0], charge_id=0, transaction_id=ledger_id, payout_transaction_id=ledgerModelService.payout_trans_id, tax_amount=0, charge_type="tax")
                tax_charges.save()
            ...
            #  tax_ledger.save("Merchant Id :: "+str(self.merchant_id),self.client_ip_address)
                ...
                for i in charge[1]:
                    ...
                    charge_ledger = Charge_Breaking_model_services(
                        charge_amount=i[0], charge_id=taxes[1][irt][0], transaction_id=ledger_id, payout_transaction_id=ledgerModelService.payout_trans_id, tax_amount=taxes[1][irt][1], charge_type="charge")
                    charge_ledger.save()
                    ...
                    irt += 1
                if mode_rec == "UPI":
                    request_model = paytm_request_model.Payment_Request_Model(transfer_mode=payoutrequestmodel.mode, subwalletGuid=const.paytm_subwalletGuid,
                                                                              orderId=ledgerModelService.payout_trans_id, beneficiaryVPA=payoutrequestmodel.upiId, amount=payoutrequestmodel.amount, purpose="OTHERS")

                else:
                    request_model = paytm_request_model.Payment_Request_Model(beneficiaryName=payoutrequestmodel.beneficiaryName, transfer_mode=payoutrequestmodel.mode, subwalletGuid=const.paytm_subwalletGuid,
                                                                              orderId=ledgerModelService.payout_trans_id, beneficiaryAccount=payoutrequestmodel.beneficiaryAccount, beneficiaryIFSC=payoutrequestmodel.beneficiaryIFSC, amount=payoutrequestmodel.amount, purpose="OTHERS")

                log_model = Log_Model_Service(json=str({"merchant_id": self.merchant_id}), log_type="Paytm_Request",
                                              server_ip_address=const.server_ip, client_ip_address=self.client_ip_address, full_request=str(request_model.to_json()))
                ...
                class ServiceThread2(threading.Thread):
                    def run(self):
                        log = Log_Model_Service(json=str({"merchant_id": merchant_id_temp}), log_type="Thread", client_ip_address=client_ip_address_temp,
                                                server_ip_address=const.server_ip, remarks="Running service thread on webhook apis for merchant id :: " + merchant_id_temp)
                        log.save()
                ...
                if(response_model.status == "ACCEPTED"):
                    ledgerModelService.update_status(
                        id, "Proccesing", client_ip_address=self.client_ip_address, created_by="Merchant ID :: "+str(self.merchant_id))
                    client_ip_address_temp = self.client_ip_address
                    merchant_id_temp = self.merchant_id
                    thread = None
                    ...
                    class ServiceThread(threading.Thread):
                        def run(self):
                            log = Log_Model_Service(json=str({"merchant_id": merchant_id_temp}), log_type="Thread", full_request={"orderId": ledgerModelService.payout_trans_id},
                                                    client_ip_address=client_ip_address_temp, server_ip_address=const.server_ip, remarks="Running service thread on paytm enquiry api for merchant id :: " + merchant_id_temp)
                            logid = log.save()
                            time.sleep(40)
                            checksum = paytmchecksum.generateSignature(json.dumps(
                                {"orderId": ledgerModelService.payout_trans_id}), const.paytm_merchant_key)
                            response = requests.post(const.paytm_link_enquiry, json={"orderId": ledgerModelService.payout_trans_id}, headers={
                                                     "Content-type": "application/json", "x-mid": const.paytm_merchant_id, "x-checksum": checksum})
                            log.update_response(logid, response.text)
                            print(response.json())
                            if response.json()['status'] == "SUCCESS":
                                ledgerModelService.update_status(id, "Success", client_ip_address_temp, "Merchant :: "+str(
                                    merchant_id_temp), response.json()['result']['rrn'])
    ...
    def excuteICICI(self):
        log = Log_Model_Service(json=str({"merchant_id": self.merchant_id}), log_type="excuting ICICI service",
                                client_ip_address=self.client_ip_address, server_ip_address=const.server_ip, created_by=self.client_code)
        log.save()
        try:
            clientModelService = Client_model_service.Client_Model_Service()
            ...
            if(valid):
                ...
                header = icic_payment_model.Header_Request(Username=bank_api.icici.icic_details(
                )["iciciImpsUserName"], Password=bank_api.icici.icic_details()["Password"])
                body = icic_payment_model.Body_Request(IFSCCode=payoutrequestmodel.ifscCode, remiMobileNumber=payoutrequestmodel.payeeMob, remarks="payment", customerID=bank_api.icici.icic_details(
                )["iciciImpsUserName"], customerReferenceNumber=payoutrequestmodel.clientTransactionId, debitAccountNumber=bank_api.icici.icic_details()["debitAccount"], creditAccountNumber=payoutrequestmodel.creditAccountNumber, transactionAmount=payoutrequestmodel.txnAmount)
                response = requests.post(
                    headers=header.to_Json(), json=body.to_json())
                response_obj = icici_response.Response_Model.from_json(
                    json=response.json())
    ...
    def excuteIDFC(self):
        log = Log_Model_Service(log_type="excuting IDFC service", client_ip_address=self.client_ip_address,
                                server_ip_address=const.server_ip, created_by=self.client_code)
        log.save()
        try:
            clientModelService = Client_model_service.Client_Model_Service()
            clientModel = clientModelService.fetch_by_clientcode(
