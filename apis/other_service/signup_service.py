...
class Signup_Service:
    ...
    def AdminSignup(self):
        ...
        boclient = BO_user_services.BO_User_Service(name=self.user["name"],mobile=self.user["phone_number"],email=self.user["email"],roleid=self.user['role_id'],auth_key=randomstring.randomString(),auth_iv=randomstring.randomString(),username=self.user["username"],password=self.user["password"])
